...
def test_create_meeting(tmp_path):
    ...
    assert any(m["meeting_id"] == "id1" for m in data["meetings"])
...
def test_update_status(tmp_path):
    ...
    updated = manager.update_status("id1", "Transcribed", transcript_path="storage/id1.txt")
    assert updated["status"] == "Transcribed"
    assert updated["transcript_path"] == "storage/id1.txt"
    # end_timestamp remains None until Completed
    assert updated["end_timestamp"] is None
...
def test_get_and_delete(tmp_path):
    ...
    assert meeting and meeting["meeting_id"] == "id1"
